...
function Cell({ children, hidden, onClick }) {
  return (
    <td>
      <button tabIndex={hidden ? -1 : 0} onClick={onClick}>{children}</button>
    </td>
  );
}
  ...
  return (
    <div className="schedule">
      <table aria-label="Accessibility Week Schedule">
        ...
        <tbody>
          <tr>
            ...
            <Cell hidden={showDialog} onClick={open("keynote")}>Keynote</Cell>
            <Cell hidden={showDialog} onClick={open("talk1")}>Talk</Cell>
            <Cell hidden={showDialog} onClick={open("talk2")}>Talk</Cell>
            <Cell hidden={showDialog} onClick={open("talk3")}>Talk</Cell>
            <Cell hidden={showDialog} onClick={open("innovationtime")}>Innovation time</Cell>
          </tr>
          <tr>
            <th scope="row">11:00 - 12:30</th>
            <Cell hidden={showDialog} onClick={open("innovationtime")}>Innovation time</Cell>
            <Cell hidden={showDialog} onClick={open("workshop")}>Workshop</Cell>
            <Cell hidden={showDialog} onClick={open("innovationtime")}>Innovation time</Cell>
            <Cell hidden={showDialog} onClick={open("workshop")}>Workshop</Cell>
            <Cell hidden={showDialog} onClick={open("innovationtime")}>Innovation time</Cell>
          </tr>
      ...
      {current && data[current] && (
        ...
        >
          <main>
            <h1 id="dialog-label">{data[current].title}</h1>
            <p
              tabIndex={0}
